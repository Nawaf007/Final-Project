    ...
    public class StudentSectionController : Controller
    {
        ...
        public ActionResult Index(int? ids, int? idsc, int? idc, int? ide, int? idss)
        {
            ...
            else if (adtype() == "Teacher")
            {
                if (ide != null)
                {
                    var ssexams = db.SSExams.Include(s => s.Exam).Include(s => s.SectionStudent).Where(s => s.ExamId == ide && s.Exam.SectionCourse.Teacher.AspNetUser.UserName == User.Identity.Name);
                    return View(ssexams.ToList());
                }
                if (ids != null)
                {
                    var ssexams = db.SSExams.Include(s => s.Exam).Include(s => s.SectionStudent).Where(s => s.SSId == idss && s.Exam.SectionCourse.Teacher.AspNetUser.UserName == User.Identity.Name);
                    return View(ssexams.ToList());
                }
            }
        ...
        public ActionResult Create(int? ids, int? idsc, int? idc, int? ide, int? idss)
        {
            ...
            if (adtype() == "Admin")
            {
                if (ide != null)
                {
                    ...
                    var look = db.SectionStudents.Where(ss => db.SSExams.Where(sse => sse.SSId == ss.Id && sse.ExamId == ide).Count() == 0);
                ...
                if (idss != null)
                {
                    ViewBag.ExamId = new SelectList(db.Exams.Where(e => db.SSExams.Where(ss => ss.ExamId == e.Id).Count() == 0), "Id", "ExamType");
                    var look = db.Exams.Where(e => db.SSExams.Where(ss => ss.ExamId == e.Id).Count() == 0);
                    IList<SelectListItem> status = new List<SelectListItem> { };
                    foreach (Exam item in look)
                    {
                        SelectListItem m = new SelectListItem();
                        m.Text = item.Lookup.Values;
                        m.Value = item.Id.ToString();
                        status.Add(m);
                    }
                    ViewBag.ExamId = status;
            ...
            {
                if (ide != null && db.Exams.Where(e => e.SectionCourse.Teacher.AspNetUser.UserName == User.Identity.Name).Count() > 0)
                {
                    ViewBag.ExamId = new SelectList(db.Exams, "Id", "ExamType", ide);
                    ViewBag.SSId = new SelectList(db.SectionStudents.Where(ss => db.SSExams.Where(sse => sse.SSId == ss.Id && sse.ExamId == ide).Count() == 0), "Id", "Id");
                    var look = db.SectionStudents.Where(ss => db.SSExams.Where(sse => sse.SSId == ss.Id && sse.ExamId == ide).Count() == 0);
                ...
                if (idss != null && db.SectionStudents.Where(ss => ss.Section.SectionCourses.Where(sc => sc.Teacher.AspNetUser.UserName == User.Identity.Name).Count() > 0).Count() > 0)
                {
                    ViewBag.ExamId = new SelectList(db.Exams.Where(e => db.SSExams.Where(ss => ss.ExamId == e.Id).Count() == 0), "Id", "ExamType");
                    var look = db.Exams.Where(e => db.SSExams.Where(ss => ss.ExamId == e.Id).Count() == 0);
                    IList<SelectListItem> status = new List<SelectListItem> { };
            ...
            if (adtype() == "Admin")
            {
                ...
                if (idss != null)
                {
                    ...
                    foreach (Exam item in look)
                    {
                        ...
                        m.Text = item.Lookup.Values;
                        m.Value = item.Id.ToString();
                        if (m.Value == ssexam.ExamId.ToString())
                        {
                            m.Selected = true;
                        }
                        status.Add(m);
